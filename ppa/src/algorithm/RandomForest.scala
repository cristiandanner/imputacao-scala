...
object RandomForest {
  ...
  def run(idf: DataFrame, attribute: String, params: HashMap[String, Any] = null, callback: String => Double): entities.Entities.NumericImputationResult = {
    ...
    val numTrees = params("numTrees").asInstanceOf[Int]
    val featureSubsetStrategy = params("featureSubsetStrategy").asInstanceOf[String]
    ...
    val randomForestModel = org.apache.spark.mllib.tree.RandomForest.trainClassifier(trainingData.map(_._2), numClasses, categoricalFeaturesInfo, numTrees, featureSubsetStrategy, impurity, maxDepth, maxBins)

    // Evaluate model on test instances and compute test error
    val labelAndPreds = testData.filter(_._2.label == 0).map { point =>
      val prediction = randomForestModel.predict(point._2.features)
